...
function actualiza_por_aplicar(por_aplicar){
    por_aplicar = parseFloat(por_aplicar).toFixed(2);
...
$(document).ready(function () {
    ...
    if(seccion == 'pago_cliente' && accion == 'alta_cfdi_relacionado'){
        ...
        $('.monto_pagar').change(function (){
            ...
            var monto_por_aplicar = parseFloat($(this).val()).toFixed(2);
        ...
        $('#descarga').click(function () {
            ...
            $( ".factura" ).each(function( index ) {
                ...
                if( $(this).prop('checked') ) {
                    ...
                    window.setTimeout(funcion_abre , time );
                    time = time + 1000;
                    var funcion_abre = "open_window('./index.php?seccion=factura&accion=informe_gastos_pdf&session_id="+session_id+"&factura_id="+factura_id+"');";
                    window.setTimeout(funcion_abre , time );
                }
            });
        });
    ...
    if ($("#select_pais").length) {
        ...
        if ($("#contenedor_select_estado").length) {
            $('#select_pais').change(function (){
                var id = $(this).val();
                let option = genera_option($(this),'estado','pais_id',$('.estado_id'),'','');
    ...
    if(seccion == 'anticipo' && accion=='alta'){
        function calcula_total(){
            total = (monto * porcentaje_iva) + monto;
        }
        var monto = false;
        var porcentaje_iva = false;
        var total = false;
        $('.monto').change(function () {
            monto = parseFloat($(this).val());
          if(porcentaje_iva != false){
              calcula_total();
              $('.total').val(total);
          }
        });
        ...
        $('.porcentaje_iva').change(function () {
            porcentaje_iva = parseFloat($(this).val());
            if(monto != false){
                calcula_total();
                $('.total').val(total);
            }
