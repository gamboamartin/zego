...
<div class="col-md-12">
    ...
    <div class="col-md-12 row" id="insumo-selector-grabado">
        <?php
            $i = 0;
            foreach ($controlador->partidas as $partida){
                ...
                <?php echo $partida['insumo_descripcion']; ?>
                <label>Codigo SAT:</label>
                <?php echo $partida['producto_sat_codigo']; ?>
                <label>Cantidad:</label>
                <?php echo $partida['cantidad']; ?>
                <label>Unidad:</label>
                <?php echo $partida['unidad_descripcion']; ?>
                <label>Valor Unitario:</label>
                $<?php echo number_format(round($partida['valor_unitario'],2),2,'.',','); ?>
                <label>Impuesto Trasladado:</label>
                <?php echo $partida['impuesto_descripcion_trasladado']; ?>
                <label>Factor Trasladado:</label>
                <?php echo $partida['insumo_factor_trasladado']; ?>
                <label>Impuesto Trasladado:</label>
                $<?php echo number_format(round($partida['monto_impuesto_trasladado'],2),2,'.',','); ?>
                <?php
                    if($partida['monto_impuesto_retenido'] > 0){
                ?>
                <label>Impuesto Retenido:</label>
                <?php echo $partida['impuesto_descripcion_retenido']; ?>
                <label>Factor Retenido:</label>
                <?php echo $partida['insumo_factor_retenido']; ?>
                <label>Impuesto Retenido:</label>
                        $<?php echo number_format(round($partida['monto_impuesto_retenido'],2),2,'.',','); ?>
                        <?php } ?>

                <label>Total:</label>
                $<?php echo number_format(round($partida['total'],2),2,'.',','); ?>
                <a href="./index.php?seccion=factura&accion=elimina_partida&partida=<?php echo $i; ?>">
                    Elimina Partida
                </a>
        <?php
            $i++;
...
<hr>
<?php
echo $directiva->btn_enviar(12,'Finalizar','siguiente');
?>